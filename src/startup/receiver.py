...
class StreamingReceiver:
    def __init__(self, config_path: str):
        ...
        self.running = False
        self.active_pipelines: list[GStreamerInterface] = []
    ...
    def start(self, stream_types: list[StreamType], only_display: bool = False) -> bool:
        LOGGER.info("=" * 60)
        ...
        if not only_display:
            LOGGER.info(
                f"Attempting to connect to ROS IPC Server at {self.interface.ipc_target}..."
            )
            while self.running:
                try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="GStreamer Receiver Process (Process A)")
    parser.add_argument(
        "--config", type=str, default="src/config/config.yaml", help="Path to config.yaml"
    )
    ...
    parser.add_argument(
        "--only-display", action="store_true", help="Only display streams (disables IPC sending)"
    )
...
def main() -> None:
    ...
    try:
        ...
        success = receiver.start(
            stream_types=list(set(stream_types)), only_display=args.only_display
        )
        if success:
