...
class GStreamerROSReceiver(Node):
    ...
    def _launch_receiver_pipeline_with_ros(self, pipeline: 'GStreamerPipeline'):
        ...
        if appsink:
            ...
            self.get_logger().error(f"Could not find appsink '{appsink_name_map.get(pipeline.stream_type)}' in pipeline. ROS publishing will fail.")
    ...
    def _on_ros_new_sample(self, appsink: Gst.Element, pipeline: 'GStreamerPipeline'):
        ...
        try:
            ...
            if pipeline.stream_type == StreamType.DEPTH:
                if format_name in ['GRAY16_LE', 'GRAY16']:
                    ...
                    if len(data) == expected_size:
                        ...
                        self.get_logger().warning(f"Depth buffer size mismatch! Got {len(data)}, expected {expected_size}. Dropping frame.")
                ...
                if format_name in ['BGR']:
                    ...
                    if len(data) == expected_size:
                        ...
                        self.get_logger().warning(f"Color buffer size mismatch! Got {len(data)}, expected {expected_size}. Dropping frame.")
                        return Gst.FlowReturn.OK
                else:
                ...
                if format_name in ['GRAY8']:
                    ...
                    if len(data) == expected_size:
                        ...
                        self.get_logger().warning(f"Infra buffer size mismatch! Got {len(data)}, expected {expected_size}. Dropping frame.")
                        return Gst.FlowReturn.OK
                else:
            ...
            self.publishers[pipeline.stream_type].publish(ros_image_msg)

        except Exception as e:
            self.get_logger().error(f"ROS Publish Error for {pipeline.stream_type.value}: {e}", throttle_duration_sec=1.0)
            return Gst.FlowReturn.ERROR
        finally:
...
def main(args=None):
    ...
    try:
        interface = create_receiver_interface(config_path="src/config/config.yaml")
    except Exception as e:
