...
class GStreamerInterface:
    ...
    def build_y8i_split_sender_pipeline(
        ...
        # single IR from Y8I (side by side)
        ...
        protocol = self.config.network.transport.protocol  # e.g. "udp"
        server_ip = self.config.network.server.ip
        ...
        # Upstream capture for the Python splitter (kept for your feeder thread)
        v4l2_cmd = (
            f"v4l2-ctl -d {shlex.quote(device)} "
        ...
        # Common queue to avoid backpressure
        ...
        left_pipeline_str = (
            ...
            f"{enc_left} ! h264parse config-interval=-1 disable-passthrough=true ! "
            f"rtph264pay pt={pt_l} config-interval=1 ! "
            f"{q} ! {left_sink}"
        )
        ...
        right_pipeline_str = (
            ...
            f"{q} ! videoconvert ! {cpu_nv12_caps_str} ! "
            f"nvvidconv ! {nvmm_caps_str} ! "
            f"{enc_right} ! h264parse config-interval=-1 disable-passthrough=true ! "
            f"rtph264pay pt={pt_r} config-interval=1 ! "
            f"{q} ! {right_sink}"
        )
